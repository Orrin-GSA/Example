...
/**
 * NOTE: This one is shared with enhancements. Enhancements' primary difference is it should be able to copy from the associated RPA project
 * ISSUE: Shouldn't an ENH just mimic the original projects requirements? Be it RPA or Script?
 * @param {Object} props
...
*/
function RpaMilestonePreview({ project, status, stage }) {
    ...
    const badgePtaClass = (selectedMilestone?.pta_complete) ? 'milestone-done mx-2 px-2' : 'milestone mx-2 px-2';
    const badgeSecClass = (selectedMilestone?.security_assessment) ? 'milestone-done mx-2 px-2' : 'milestone mx-2 px-2';
    ...
    return <div className={trackerClass}>
        <div className='d-flex float-right my-1 milestones'>
            <div className={badgePtaClass} title='Privacy Threshold Assessment'>PTA</div>
            <div className={badgeSecClass} title='Security'>SEC</div>
        ...
        <div className="progress-container">
            ...
            <button title='Edit Milestones' aria-label={`Edit Milestones for ${project.name}`} className='milestone-edit' onClick={handleClickEditMStones} >
...
RpaMilestonePreview.propTypes = {
    project: PropTypes.object.isRequired,
    status: PropTypes.object.isRequired,
    stage: PropTypes.object
}

export default RpaMilestonePreview;